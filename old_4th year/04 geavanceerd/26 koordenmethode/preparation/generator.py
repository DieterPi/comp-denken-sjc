...
cases.append( ['def f( x ) : return 5 * (x - 2) * (x - 3) * (x - 4)', 2.5, 0, -7] )
cases.append( ['def f( x ) : return -5 / (x**2 + 1) + 2', -1, 3, -3] )
cases.append( ['def f( x ) : return -5 / (x**2 + 1) + 2', 3, -1, -3] )
...
for test in cases:
    ...
    print('>>> koordenmethode( f, {}, {}, 10**{})'.format( test[1], test[2], test[3] ) )
            
    # generate return value
    try:        
        f = makeF()
        print( module.koordenmethode( f, test[1], test[2], 10**int(test[3]) ) )
